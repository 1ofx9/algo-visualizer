...
export default function SortingPage() {
	...
	function arrayStatus() {
		...
		return (
			<p className="text-system-barchange">Array is not Sorted :&apos;(</p>
		);
	}
	...
	useEffect(() => {
		...
		const newHeights = new Array(arrayToSort.length);
		let hasMeasurements = false;

		for (let i = 0; i < arrayToSort.length; i++) {
			const ref = divRefs.current[i];
			if (ref?.current) {
				const currentRef = ref.current;
				newHeights[i] = currentRef.offsetHeight;
				hasMeasurements = true;

				const observer = new MutationObserver(() => {
					const updatedHeight = currentRef.offsetHeight;
					setDivHeights((prev) => {
						if (prev[i] === updatedHeight) return prev;
						const next = [...prev];
						next[i] = updatedHeight;
						return next;
					});
				});
								...
								<div className="flex flex-col">
									...
									<div className="flex gap-1">
										...
										{(() => {
											const speed =
												animationSpeed === 0 ? (
													<p className="text-system-barchange">
									...
									<TooltipTrigger asChild>
										...
										>
											<span className="text-secondary py-1">
												{divHeights[index] !== undefined &&
												divHeights[index] !== null ? (
